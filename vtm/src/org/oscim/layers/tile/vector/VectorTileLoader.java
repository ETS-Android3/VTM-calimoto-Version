...
public class VectorTileLoader extends TileLoader implements RenderStyle.Callback {
    ...
    private static float getLineScaleInternal(double y, double zoomLevel) {
        ...
        float mLineScale = (float) Math.pow(STROKE_INCREASE, zoomLevel - STROKE_MIN_ZOOM);
        if (mLineScale < 1)
            mLineScale = 1;
    ...
    public static float getLineScale(double y, double scale) {
        return getLineScaleInternal(y, Math.log(scale)/Math.log(2));
    }
