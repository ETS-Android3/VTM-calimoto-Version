...
public final class LineStyle extends RenderStyle<LineStyle> {
    ...
    public LineStyle(int stroke, float width) {
        this(0, stroke, width, Cap.BUTT, true, 0 );
    }
    
    public LineStyle(int level, int stroke, float width) {
        this(level, stroke, width, Cap.BUTT, true, 0);
    }
    
    public LineStyle(int stroke, float width, Cap cap) {
        this(0, stroke, width, cap, true, 0);
    }
    
    public LineStyle(int level, int stroke, float width, Cap cap, boolean fixed, float minWidth) {
        this(level, "", stroke, width, cap, fixed, minWidth, 0, 0,
                0, -1, 0, false, null, true, null, REPEAT_START_DEFAULT, REPEAT_GAP_DEFAULT);
    }
    ...
    public LineStyle(int level, String style, int color, float width,
        ...
        this(level, style, color, width, cap, fixed, 0, stipple, stippleColor,
                stippleWidth, fadeScale, blur, isOutline, texture, randomOffset, null, REPEAT_START_DEFAULT, REPEAT_GAP_DEFAULT);
    }
    
    public LineStyle(int level, String style, int color, float width,
                     Cap cap, boolean fixed, float minWidth,
                     int stipple, int stippleColor, float stippleWidth,
                     int fadeScale, float blur, boolean isOutline, TextureItem texture,
                     boolean randomOffset, float[] dashArray, float repeatStart, float repeatGap) {
