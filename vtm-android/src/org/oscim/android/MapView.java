...
public class MapView extends GLSurfaceView {
    ...
    static final Logger log = new Logger(MapView.class);
    ...
    public boolean onTouchEvent(android.view.MotionEvent motionEvent) {
        ...
        // consume events while animation is running to avoid disrupting animation
        if (mMap.animator().shouldBlockTouch())
            return true;

        // gesture detector has highest priority, then on touch listener
        if (mGestureDetector != null) {
            if (mGestureDetector.onTouchEvent(motionEvent))
                return true;
        }
        if (mOnTouchListener != null) {
            if (mOnTouchListener.onTouch(this, motionEvent))
                return true;
        }
    ...
    static class GLRenderer extends org.oscim.renderer.MapRenderer
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            try {
                ...
                log.warn("Version: " + versionString);
